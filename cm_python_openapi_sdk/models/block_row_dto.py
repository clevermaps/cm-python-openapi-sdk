...
class BlockRowDTO(BaseModel):
    ...
    __properties: ClassVar[List[str]] = ["type", "blocks"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['blockRow']):
            raise ValueError("must be one of enum values ('blockRow')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "type": obj.get("type"),
            "blocks": [IndicatorLinkDTO.from_dict(_item) for _item in obj["blocks"]] if obj.get("blocks") is not None else None
        })
        return _obj
